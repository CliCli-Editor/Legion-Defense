...
local function toObj(table)
    ...
    for key, value in pairs(table) do
        ...
        if (t == "table") then
            ...
            local o = toObj(data["__properties"]);
            obj[key] = o;
...
local function toRaw(target)
    local observed = getmetatable(target)
    return toObj(observed["__properties"]);
end

local function reactive(target)
    if (type(target) ~= "table") then
        if __DEV__ then
            warn("target cannot be made reactive: ", tostring(target), debug.traceback())
        end
        return target
    end
