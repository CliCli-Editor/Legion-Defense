...
local schedulerFrameSync = require("reactivity.schedulerFrameSync")
local queueJobFrameSync, queuePostFlushCbFrameSync = schedulerFrameSync.queueJob, schedulerFrameSync.queuePostFlushCb
...
local function traverse(value, seen,deep)
    if seen == nil then
        seen = {}
    end
    ...
    for key in pairs(value) do
        traverse(value[key], seen,deep)
    end
    if deep then
        local observed = getmetatable(value)
        local target = value
        if observed and observed.__safeTable then
            target=observed.__safeTable.raw
            observed = getmetatable(target)
        end
        if observed and observed[ReactiveFlags.IS_REACTIVE] then
...
local function doWatch(source, cb, options)
    ...
    if cb and deep then
        ...
        getter = function()
            return traverse(baseGetter(),nil,true)
        end
    end
    ...
    if flush == "sync" then
        ...
        --         end
        --     end
    elseif flush == "frameSync" then
        scheduler = function(job)
            queuePostFlushCbFrameSync(job, instance and instance.suspense)
        end
    else
        scheduler = function(job)
