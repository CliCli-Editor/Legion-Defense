...
function Blackboard:__init(clock, parent)
    _blackboardId = _blackboardId + 1;
    ---@type number
    self._id = _blackboardId
    self._clock = clock
    ---@type {[string]: any}
    self._data = NPBehave.Util.OrderedTable()
    ---@type {[string]: fun(type: NPBehaveBlackboardType, value: any)[]}
    self._observers = NPBehave.Util.OrderedTable() -- 观察者, 实际执行的函数
    self._isNotifying = false
    ---@type {[string]: fun(type: NPBehaveBlackboardType, value: any)[]}
    self._addObservers = NPBehave.Util.OrderedTable()
    ---@type {[string]: fun(type: NPBehaveBlackboardType, value: any)[]}
    self._removeObservers = NPBehave.Util.OrderedTable()
    ...
    ---@type {[number]: NPBehave.Blackboard} 模拟`hashset`
    self._children = NPBehave.Util.OrderedTable()
    return self
end
...
function Blackboard:Enable()
    if self._parentBlackboard then
        self._parentBlackboard._children[self._id] = self
    end
end
...
function Blackboard:Disable()
    if self._parentBlackboard then
        self._parentBlackboard._children[self._id] = nil
    end
    if self._clock then
...
function Blackboard:NotifyObservers()
    ...
    for id, child in pairs(self._children) do
        for _, notification in ipairs(self._notifications) do
