...
local function replaceSouceMap(file, line)
    if not y3.game.is_debug_mode() or _G.__TS__sourcemap == nil then
        return file, line
    end

    local fileSourceMap = _G.__TS__sourcemap[file]
    if fileSourceMap == nil then
        return file, line
    end
    ...
    if type(data) == "number" then
        return string.sub(file, 1, -4) .. 'ts', data
    end
...
function M:build(level, ...)
    ...
    local sourceStr = '?'
    xpcall(function ()
        local info =y3.game.is_debug_mode() and  debug.getinfo(6, 'Sl') or debug.getinfo(6, 'Sl')
        if info.currentline == -1 then
        sourceStr = '?'
    else
        sourceStr = ('%s:%d'):format(replaceSouceMap(info.short_src, info.currentline))
        end
    end,function ()end)

    if y3.game.is_debug_mode() then
        sourceStr = string.gsub(sourceStr, "^.*/script/", "")
    else
        sourceStr = string.gsub(sourceStr, "^virtual_script/", "")
    end
    local strLen = string.len(sourceStr);
    if strLen < 50 then
        sourceStr = string.rep(' ', 50 - strLen) .. sourceStr
    end
