...
____exports.DDecPetHaloPage = __TS__Class()
local DDecPetHaloPage = ____exports.DDecPetHaloPage
DDecPetHaloPage.name = "DDecPetHaloPage"
__TS__ClassExtends(DDecPetHaloPage, Dialog)
function DDecPetHaloPage.prototype.____constructor(self, ...)
    Dialog.prototype.____constructor(self, ...)
    self.data = {selectIndex = 0}
end
function DDecPetHaloPage.prototype.onCreate(self)
    self:initView()
end
function DDecPetHaloPage.prototype.initView(self)
    return __TS__AsyncAwaiter(function(____awaiter_resolve)
        self:bindVisible(
            "root",
            function() return localData.decoration.tabIndex == DecorationType["PetHalo_宠物光环"] end
        )
...
function DDecPetHaloPage.prototype.onShow(self)
end
function DDecPetHaloPage.prototype.onHide(self)
    self:getUI("root.center.model"):set_ui_model_id(0)
end
function DDecPetHaloPage.prototype.initDetails(self)
    local root = "root.right.list"
    local childrens = self:getUI(root):get_childs()
    local list = self:compute(function()
        local list1 = petHaloConfigList
        local list2 = {}
        do
            local index = 0
            while index < #list1 do
                do
                    ...
                    local info = archive.petData.halos["" .. tostring(element.id)]
                    list2[#list2 + 1] = {cfg = element, select = archive.petData.cur_halo == element.id, unlock = info == true}
                end
                ::__continue11::
                index = index + 1
            end
    ...
    self:bindClick(
        ...
        function()
            if steamUtils:isSteam() then
                ...
                if data ~= nil then
                    steamUtils:gotoShop(17, data.cfg.id)
                end
            else
    ...
    self:bindClick(
        ...
        function()
            if list.value[self.data.selectIndex + 1].select then
                heroSkinManager:replacePetHalo(0)
                createUnitController.changeHalo:execute(LOCAL.PLAYER_ID, 0)
            else
                heroSkinManager:replacePetHalo(list.value[self.data.selectIndex + 1].cfg.id)
                createUnitController.changeHalo:execute(LOCAL.PLAYER_ID, list.value[self.data.selectIndex + 1].cfg.wing)
            end
        end
    )
    self:watch(
...
function DDecPetHaloPage.prototype.change(self, wing)
    self:getUI("root.center.model"):set_ui_model_id(0)
